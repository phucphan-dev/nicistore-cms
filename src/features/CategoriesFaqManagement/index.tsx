...
import { changeStatusFaqCategoryManyIdService, deleteFaqCategoriesService, getAllFaqCategoriesService } from 'common/services/faqs';
import { FaqCategoryTypes } from 'common/services/faqs/types';
...
export type NewsDataTypes = {
  ...
  publishedAt: string;
  status: number;
  locale: FaqCategoryTypes['translations'];
};

const convertNewsData = (data: FaqCategoryTypes[]): NewsDataTypes[] => {
  if (!data.length) return [];
  return data.map((val) => ({
    id: val.faqCategoryData.id,
    title: val.translations.vi ? val.translations.vi.name : val.translations.en.name,
    publishedAt: val.faqCategoryData.createdAt,
    status: val.faqCategoryData.status,
    locale: Object.fromEntries(
      Object
        .entries(val.translations)
        .map(([k, o]) => [k, { ...o, id: val.faqCategoryData.id, }])
    )
  }));
};

const CategoriesFaqManagement: React.FC<ActiveRoles> = ({
  ...
  const queryKey = ['categoryFaq-get', keyword, currentPage, currentView, selectedFilterList];
  ...
  const { mutate: deleteMutate, isLoading: deleteLoading } = useMutation(
    ['categoryFaq-delete'],
    async (ids: number[]) => deleteFaqCategoriesService({ ids }),
    {
      onSuccess: () => {
  ...
  const { mutate: changeStatusMutate, isLoading: changeStatusLoading } = useMutation(
    ...
    ) => changeStatusFaqCategoryManyIdService(data.ids, data.status),
    {
      onSuccess: () => {
  ...
  const columns: ColumnsType<NewsDataTypes> = [
    ...
    // --- Tiêu đề
    {
      ...
      render: (_: string, data: any) => (
        <Typography.Text
          onClick={() => roleView && navigate(`${ROUTE_PATHS.FAQ_CATEGORY_DETAIL}?id=${data.id}&locale=${defaultWebsiteLanguage}`)}
  ...
  const filterFields: FilterDataProps[] = useMemo(
    () => mappingFilterFields('faqCategory', advancedFilter),
    [advancedFilter]
  );
  ...
  return (
    <>
      <HeaderPage
        fixed
        title={t('sidebar.faqCategory')}
        rightHeader={(
          <Button type="primary" disabled={!roleCreate} onClick={() => navigate(`${ROUTE_PATHS.FAQ_CATEGORY_DETAIL}?locale=${defaultWebsiteLanguage}`)}>
            <PlusOutlined />
            {t('system.create')}
          </Button>
        )}
      ...
      <div className="t-mainlayout_wrapper">
        <PageTable
          ...
          handleEditPage={(id, _, locale) => {
            navigate(`${ROUTE_PATHS.FAQ_CATEGORY_DETAIL}?id=${id}&locale=${locale}`);
          }}
          ...
          statusDataTable={{
            ...
            changeStatusLoading,
            canChangeStatusApprove: getPermission(rolesUser, roles.FAQ_CATEGORY_APPROVED),
          }}
        />
      </div>
    </>
...
export default CategoriesFaqManagement;

...
import { createProductColorService, getProductColorByIdService, updateProductColorByIdService } from 'common/services/products';
import { CreateUpdateProductColorTypes } from 'common/services/products/types';
import { ROUTE_PATHS } from 'common/utils/constant';

type ProductColorsFormTypes = {
  status: number;
  displayOrder: number;
  name: string;
  code: string;
...
const productColorsDetailSchema = yup.object().shape({
...
const ProductColorsDetail: React.FC = () => {
  /* Hooks */
  ...
  const method = useForm<ProductColorsFormTypes>({
    resolver: yupResolver(productColorsDetailSchema),
    defaultValues
  });
  ...
  /* Queries */
  const { data: productColorData, isLoading } = useQuery(
    ['getProductColorDetail', idParams],
    () => {
      if (idParams) {
        return getProductColorByIdService(idParams);
      }
      return undefined;
    }
  );
  ...
  const { mutate: updateMutate, isLoading: updateLoading } = useMutation(
    ...
    }) => updateProductColorByIdService(data.id, data.params),
    {
      onSuccess: () => {
        message.success(t('message.updateSuccess'));
        queryClient.invalidateQueries(['getProductColorDetail', currentLang, idParams]);
      },
      onError: () => {
  ...
  const { mutate: createMutate, isLoading: createLoading } = useMutation(
    ...
    createProductColorService,
    {
      onSuccess: () => {
        ...
        navigate(`${ROUTE_PATHS.PRODUCT_COLORS_MANAGEMENT}`);
      },
      onError: () => {
  ...
  useEffect(() => {
    if (productColorData) {
      setStatus(productColorData.status);
      const {
        name, code, displayOrder
      } = productColorData;
      const objDefault: ProductColorsFormTypes = {
        name,
        code,
        displayOrder,
        status: productColorData.status,
      };
      method.reset(objDefault);
    } else {
      method.reset(defaultValues);
    }
  }, [productColorData, currentLang, method]);
            ...
            <Row gutter={16}>
              ...
              <Col xxl={6} xl={8} lg={8}>
                ...
                <ManagementInfo
                  ...
                  createdDate={productColorData ? moment(productColorData.createdAt).fromNow() : ''}
                  createdBy="Admin"
                  lastUpdated={productColorData ? moment(productColorData.updatedAt).fromNow() : ''}
                  lastUpdatedBy="Admin"
...
export default ProductColorsDetail;

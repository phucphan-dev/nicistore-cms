...
import { changeStatusFaqManyIdService, deleteFaqService } from 'common/services/faqs';
import { getAllProductCategories } from 'common/services/products';
  ...
  const columns: ColumnsType<ProductCategoryTypes> = [
    ...
    // --- Tiêu đề
    {
      ...
      render: (_: string, data: any) => (
        <Typography.Text
          onClick={() => navigate(`${ROUTE_PATHS.FAQ_DETAIL}?id=${data.id}&locale=${defaultWebsiteLanguage}`)}
  ...
  return (
    <>
      <HeaderPage
        ...
        rightHeader={(
          <Button type="primary" onClick={() => navigate(`${ROUTE_PATHS.FAQ_DETAIL}?locale=${defaultWebsiteLanguage}`)}>
            <PlusOutlined />
            {t('system.create')}
          </Button>
        )}
